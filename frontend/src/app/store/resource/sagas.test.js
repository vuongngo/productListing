...
describe('readResourceList', () => {
  ...
  it('calls success', () => {
    const detail = { list: [1, 2, 3], meta: { page: 1 } };
    const generator = sagas.readResourceList(api, payload, meta);
    expect(generator.next().value)
      .toEqual(call([api, api.get], `/${resource}`, payload));
    expect(generator.next(detail).value)
      .toEqual(put(actions.resourceListReadSuccess(resource, detail.list, payload, thunk)));
    expect(generator.next().value)
      .toEqual(put(actions.resourceListReadMeta(resource, detail.meta, payload)));
  });
